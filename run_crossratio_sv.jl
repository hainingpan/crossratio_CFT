...
function run_dw_t(L::Int,p_ctrl::Float64,p_proj::Float64,seed::Int)
    ...
    SA,SB,SAB=CT.bipartite_mutual_information_self_average(ct,0,sv=true)
    return Dict("SA"=>SA,"SB"=>SB,"SAB"=>SAB)
end
...
function main_interactive(L::Int,p_ctrl::Float64,p_proj::Float64,seed::Int)
    ...
    filename = "MPS_(0,1)_L$(args["L"])_pctrl$(@sprintf("%.3f", args["p_ctrl"]))_pproj$(@sprintf("%.3f", args["p_proj"]))_s$(args["seed"])_MI1_8_sv.json"
